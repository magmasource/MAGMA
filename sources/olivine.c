      ...
      )
{
  ...
  if ( (t != tOld)       || (p != pOld) || 
    ...
    while ( (ABS(sNew[0]-sOld[0]) > 100.0*DBL_EPSILON) || 
            ...
            (ABS(sNew[3]-sOld[3]) > 100.0*DBL_EPSILON) ) {
      double s[NS];
...
/*             1111111111222222222233333333334444444444555555555566666666667
  ...
  if (mask & FIRST) {
    ...
    (void) sprintf(n, "%4.2f", totMg);  for (i=0; i<4; i++) string[ 9+i] = n[i];
    (void) sprintf(n, "%4.2f", totFe2); for (i=0; i<4; i++) string[17+i] = n[i];
    (void) sprintf(n, "%4.2f", totMn);  for (i=0; i<4; i++) string[23+i] = n[i];
    (void) sprintf(n, "%4.2f", totCo);  for (i=0; i<4; i++) string[29+i] = n[i];
    (void) sprintf(n, "%4.2f", totNi);  for (i=0; i<4; i++) string[35+i] = n[i];
